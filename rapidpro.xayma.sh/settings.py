...
# -----------------------------------------------------------------------------------
HOSTNAME = env('DOMAIN_NAME', 'rapidflow.visimaxonline.com')
...
BRANDING = {
    'rapidflow.io': {
        'slug': env('BRANDING_SLUG', 'rapidflow'),
        'name': env('BRANDING_NAME', 'RapidFlow'),
        'org': env('BRANDING_ORG', 'RapidFlow'),
        'colors': dict([rule.split('=') for rule in env('BRANDING_COLORS', 'primary=#008080').split(';')]),
        'styles': ['brands/rapidpro/font/style.css'],
        'welcome_topup': 1000,
        'email': env('BRANDING_EMAIL', 'join@rapidpro.io'),
        'support_email': env('BRANDING_SUPPORT_EMAIL', 'join@rapidpro.io'),
        'link': env('BRANDING_LINK', 'https://app.rapidflow.io'),
        'api_link': env('BRANDING_API_LINK', 'https://api.rapidflow.io'),
        'docs_link': env('BRANDING_DOCS_LINK', 'http://docs.rapidflow.io'),
        'domain': HOSTNAME,
        ...
        'allow_signups': env('BRANDING_ALLOW_SIGNUPS', 'off') == 'on',
        'tiers': dict(import_flows=0, multi_user=0, multi_org=0),
        'bundles': [],
        'welcome_packs': [dict(size=5000, name="Demo Account"), dict(size=100000, name="UNICEF Account")],
        'description': _("Visually build nationally scalable mobile applications from anywhere in the world."),
        'credits': _("Copyright &copy; 2012-%s UNICEF, Nyaruka, and individual contributors. All Rights Reserved. Custom service hosted by VisiMax" % (
            datetime.now().strftime('%Y')
        ))
    }
}
DEFAULT_BRAND = 'rapidflow.io'
